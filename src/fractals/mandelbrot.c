...
/*   Updated: 2024/03/04 18:51:19 by tunsal           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_iter_escape	is_in_set(t_frac *frac)
{
	...
	while (i < frac->max_iter)
	{
		...
		if ((z.real * z.real + z.imag * z.imag) > MANDEL_DIVERGE_DIST)
			break ;
		++i;
	}
...
double default_min_x_range, double default_min_y_range)
{
	...
	x_shift_start_multiplier = MANDEL_X_START / default_min_x_range;
	x_shift_end_multiplier = MANDEL_X_END / default_min_x_range;
...
void	mandelbrot_init(t_frac *frac)
{
	...
	frac->max_iter = MANDEL_MAXITER;
	default_min_x_range = MANDEL_X_END - MANDEL_X_START;
	default_min_y_range = -(MANDEL_Y_END - MANDEL_Y_START);
